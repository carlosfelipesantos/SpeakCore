...
classDiagram
    ...
    class Turma {
        ...
        +int AnoLetivo
        +int CapacidadeMax
        +Enum Nivel (Basico, Intermediario, Avancado)
        +DateTime DataInicio
        +DateTime? DataFim
        +int DisciplinaId
        +int ProfessorId
...
6. Listar Registros 
   `GET /api/Aluno/Todos`, `/api/Turma/Todos`, `/api/Professor/Todos`, `/api/Disciplina/Todos`
    `GET /api/Aluno/{id}`, `/api/Turma/{id}`, `/api/Professor/{id}`, `/api/Disciplina/{id}`    
